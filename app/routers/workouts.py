...
def create_workout(
    ...
    trainer_db = db.query(User).filter(User.username == current_user["username"]).first()
    if not trainer_db:
        raise HTTPException(status_code=404, detail="Treinador não encontrado no banco de dados.")
...
def get_my_workouts_as_student(
    ...
    student_db = db.query(User).filter(User.username == current_user["username"]).first()
    if not student_db:
        raise HTTPException(status_code=404, detail="Aluno não encontrado.")
